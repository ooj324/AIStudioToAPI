...
class StatusRoutes {
    ...
    setupRoutes(app, isAuthenticated) {
        ...
        app.get("/api/status", isAuthenticated, async (req, res) => {
            ...
            const hasChanges = await this.serverSystem.authSource.reloadAuthSources();
        ...
        app.post("/api/accounts/deduplicate", isAuthenticated, async (req, res) => {
            try {
                ...
                if (removedIndices.length > 0) {
                    await authSource.reloadAuthSources();
                }
        ...
        app.post("/api/accounts/batch/download", isAuthenticated, async (req, res) => {
            ...
            try {
                // Build archive entries from memory cache
                const entries = [];
                for (const idx of validIndices) {
                    const data = authSource.getAuth(idx);
                    if (data) {
                        entries.push({ data: JSON.stringify(data, null, 2), name: `auth-${idx}.json` });
                    }
                }
                ...
                // Add entries to archive from memory
        ...
        app.post("/api/files/batch", isAuthenticated, async (req, res) => {
            ...
            try {
                ...
                // Parse content list
                const parsedList = files.map(f => {
                    if (!f) return null;
                    return typeof f === "string" ? JSON.parse(f) : f;
                });

                const results = await this.serverSystem.authSource.addAuthBatch(parsedList);
